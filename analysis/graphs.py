...
time_elapsed = np.array([]) #the X dim
num_light_chunks = plotit[0][9][0].shape[0] #dim for light means
ttl_time = 0
for ts in xrange(num_light_chunks):
    time_elapsed = np.append(time_elapsed, ttl_time)
    ttl_time += 0.2

fig13 = plt.figure()
mx = fig13.add_subplot(111)
mx.plot(time_elapsed, plotit[0][9][0])
plt.show()
