...
new_dnd<-rbind(new_dnd, c(0, date_and_degrees[,c(1)][2], date_and_degrees[,c(1)][3]))
count<-1
for(i in 2:NCOL(date_and_degrees)) {
 ...
 if(!is.na(curTS >= nextTS) && curTS >= nextTS) {
   new_dnd<-rbind(new_dnd, c(normalized_time_interval * count, date_and_degrees[,c(i)][2], date_and_degrees[,c(i)][3]))
